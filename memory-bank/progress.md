...
[x] 同源字形、成对拼音轨与满页空白模板修复完成：Hanzi Writer 母字统一、双行逐行配轨、小学拼音基线、6 块/页尾页补齐全部落地

[x] 固定两行完整笔顺与补空格重构完成：12 列统一尺寸、母字/全笔顺/空白格补齐 24 格、左对齐、加载门禁与配置清理全部落地
...
[x] 产品经理出版级视觉打磨完成：12px 半粗拼音与声调保护底、经典墨蓝渐进笔顺、柔和灰蓝骨架、`6 8` 稀疏米字格辅助线及 0.25mm 行距全部落地
...
- 2026-07-11: 已完成固定两行完整笔顺与补空格重构：每字统一为固定 12 列，按 `max(24, ceil((1 + 总笔画数) / 12) × 12)` 顺序排布母字、全部笔顺并以空白练习格补齐完整行；“两/哪/宽/睛”分别得到 16/14/13/10 个空白格。删除动态缩小与居中，拼音和字格统一左对齐；笔顺统一预载并在完成前禁止打印，加载失败可见，描红颜色读取配置。作废的 `traceCellsCount` / `emptyCellsCount` 已从源码和 memory 契约清除。黑名单审计、公式断言、`git diff --check` 与 `npm run build` 通过；生产包 CSS 24.07 kB（gzip 5.31 kB）、JS 253.56 kB（gzip 77.97 kB），等待人类刷新验收。
