...
## 当前所处阶段（2026-07-11 生字拼音唯一首格修复完成）
已修复选中生字后每条田字格行首格重复显示拼音的问题。根因是 `CharacterPracticeBlock` 为同一生字块内的每个网格行都向 `PinyinTrack` 传入相同 `character`；现仅在 `gridRowIndex === 0` 时传入该字，后续笔顺行与空白田字格行继续保留四线三格轨道，但不再输出拼音文字。因此每个所选生字只有其第一个田字格上方显示一次拼音。

`npm run build`、黑名单审计、`git diff --check` 与静态沙箱内嵌脚本检查一次通过；Vite 转换 21 个模块，生成 CSS 22.79 kB（gzip 5.10 kB）与 JS 250.09 kB（gzip 77.27 kB）。

## 当前所处阶段（2026-07-11 关闭四线三格后的 A4 满页修复完成）
已修复关闭左侧“显示四线三格”后分页容量仍固定沿用带轨道高度、导致田字格底部大量留白的问题。`src/components/PreviewContainer.tsx` 新增 `getPracticeRowsPerPage(showPinyin)`，按四线三格的真实显隐高度动态计算每页双行练字块容量：开启时保持 6 块，关闭时增至 8 块；分页切片与尾页空白模板补齐共用同一结果。成对验证面 `test-print.html` 已同步改为配置驱动的动态容量。

`npm run build`、`git diff --check`、静态沙箱内嵌脚本检查与 A4 物理高度断言一次通过。断言确认开启时 6 块占用 272.75mm，关闭时 8 块占用 251.75mm，继续增加一块会超过 273mm 可打印高度。in-app Browser 对 `http://127.0.0.1:5173` 的访问被其安全策略明确拒绝，因此未执行截图和交互自动化，也未绕过或切换其他浏览器。

## 当前所处阶段（2026-07-11 Bug1 模板配置与选字状态解耦完成）
已修复选字后右侧补齐模板四线三格消失的问题。根因是 `BlankPracticeRow` 通过 `data.length === 0` / `showPinyinTrack` 将四线三格渲染错误绑定到“当前页是否有已选汉字”；现已删除该选字状态分支，所有真实汉字行与补齐空白模板行统一挂载 `PinyinTrack`，轨道是否显示只读取左侧 `config.showPinyin`。田字格/米字格继续只读取 `config.showGrid` 与 `config.gridType`，不受 `selectedCharIds` 影响。`test-print.html` 已同步同形逻辑。

`npm run build` 与 `git diff --check` 一次通过，Vite 转换 21 个模块，生成 CSS 22.79 kB（gzip 5.10 kB）与 JS 249.96 kB（gzip 77.21 kB）。localhost 浏览器回归：未选字时为 12 条四线三格轨道、12 行字格、144 个单元格；选中“两”后仍为 12 条轨道、12 行字格、144 个单元格，控制台 0 error/warn，证明模板结构不再随选字动态变化。

## 当前所处阶段（2026-07-11 首页四线三格状态一致性修复完成）
已修复首页 0 个汉字时“显示四线三格”开关开启、右侧却无轨道的状态不一致。`BlankPracticeRow` 新增显式 `showPinyinTrack` 边界：仅首页空模板通过 `data.length === 0` 显示空四线三格；有字页面尾部自动补齐的纯空白块仍保持无轨道。`test-print.html` 已用同形参数同步该分支。

浏览器实测：首页默认开启时显示 12 条四线三格轨道，关闭后为 0，重新开启恢复 12；选择“两”后仅真实汉字双行块显示 2 条轨道，5 个尾部补齐块继续无轨道。控制台无错误或警告。静态内嵌脚本、黑名单审计、`git diff --check` 与 `npm run build` 一次通过，Vite 转换 21 个模块，生成 CSS 22.79 kB（gzip 5.10 kB）与 JS 250.02 kB（gzip 77.23 kB），等待人类刷新验收。

## 当前所处阶段（2026-07-11 侧栏预设清理与四线三格配置归类完成）
已彻底删除“场景化一键预设”功能区及其 `PresetId`、预设选项、组件、激活状态和 `applyPreset` 配置分支，不保留隐藏入口或失效逻辑。侧栏现在从“字帖内容”直接开始，内容说明同步收敛为教材生字与练习内容管理。

原“显示拼音”开关已更名为“显示四线三格”，并移动到“格子样式”卡片首项；Store 默认 `showPinyin: true` 保持不变。浏览器实测默认开启时选中字有 2 条拼音轨，关闭后降为 0，重新开启恢复为 2；预设标题零残留，控制台无错误或警告。`git diff --check` 与 `npm run build` 一次通过，Vite 转换 21 个模块，生成 CSS 22.79 kB（gzip 5.10 kB）与 JS 249.90 kB（gzip 77.20 kB），等待人类刷新验收。

## 当前所处阶段（2026-07-11 最终验收微调完成）
已完成最终验收微调：拼音轨不再绘制第四条底线，田字格自身的 `1px solid #000000` 顶边成为唯一接缝；轨道与田字格实测边界坐标完全一致，接缝差值为 `0px`，彻底消除双线与亚像素缝隙。

自动补齐的纯空白练习行现完全跳过 Pinyin Rail，仅保留干净的双行田字格。`src/components/PreviewContainer.tsx` 与 `test-print.html` 已同步；空状态浏览器实测为 6 个双行空白块、12 行田字格、0 条拼音轨，选中“两”后拼音轨正常显示且与字格无缝衔接。静态内嵌脚本检查、黑名单审计、`git diff --check` 与 `npm run build` 一次通过，生产构建生成 CSS 23.68 kB（gzip 5.20 kB）与 JS 251.94 kB（gzip 77.65 kB），浏览器控制台无警告或错误，等待人类刷新线上正式验收终极版本。

## 当前所处阶段（2026-07-11 拼音字体与首格显示优化完成）
已完成拼音字体族的少儿书写规范优化（单层 ɑ），并完成每行仅首个母字上方显示拼音文本的减负逻辑。

`src/components/PreviewContainer.tsx` 与成对渲染面 `test-print.html` 已同步将拼音字体强制规范为 `"Century Gothic", "Comic Sans MS", "KaiTi", "PingFang SC", sans-serif`。两侧拼音轨均仅在 `cellIndex === 0` 时写入具体拼音，后续格子文本为空但继续保留完整四线三格轨道背景。

黑名单与旧字体残留审计、`git diff --check`、静态沙箱内嵌脚本检查均通过；`npm run build` 一次通过，Vite 转换 21 个模块，生成 CSS 23.76 kB（gzip 5.22 kB）与 JS 252.24 kB（gzip 77.70 kB），等待人类刷新线上地址验收。
